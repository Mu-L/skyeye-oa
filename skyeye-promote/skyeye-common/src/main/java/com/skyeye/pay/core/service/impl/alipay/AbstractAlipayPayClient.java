...
/**
 * @ClassName: AbstractAlipayPayClient
...
public abstract class AbstractAlipayPayClient extends AbstractPayClient<AlipayPayClientConfig> {
    ...
    public PayOrderRespDTO doParseOrderNotify(Map<String, String> params, String body) throws Throwable {
        ...
        if (MapUtil.getDouble(bodyObj, "refund_fee", 0D) > 0) {
            status = PayOrderStatusResp.REFUND.getKey();
        }
    ...
    private static Integer parseStatus(String tradeStatus) {
        return Objects.equals("WAIT_BUYER_PAY", tradeStatus) ? PayOrderStatusResp.WAITING.getKey()
            : StrUtil.equalsAny(tradeStatus, "TRADE_FINISHED", "TRADE_SUCCESS") ? PayOrderStatusResp.SUCCESS.getKey()
            : Objects.equals("TRADE_CLOSED", tradeStatus) ? PayOrderStatusResp.CLOSED.getKey() : null;
    }
    ...
    protected PayRefundRespDTO doUnifiedRefund(PayRefundUnifiedReqDTO reqDTO) throws AlipayApiException {
        ...
        if (!response.isSuccess()) {
            ...
            if (StrUtil.equalsAny(response.getSubCode(), "ACQ.SYSTEM_ERROR", "SYSTEM_ERROR")) {
    ...
    protected PayRefundRespDTO doGetRefund(String outTradeNo, String outRefundNo) throws AlipayApiException {
        ...
        if (!response.isSuccess()) {
            ...
            if (StrUtil.equalsAny(response.getSubCode(), "TRADE_NOT_EXIST", "ACQ.TRADE_NOT_EXIST")) {
    ...
    protected PayTransferRespDTO doUnifiedTransfer(PayTransferUnifiedReqDTO reqDTO) throws AlipayApiException {
        ...
        if (!Objects.equals(config.getMode(), AlipayPayClientConfig.MODE_CERTIFICATE)) {
            throw new CustomException("支付宝单笔转账必须使用公钥证书模式");
        }
        ...
        if (!response.isSuccess()) {
            ...
            if (StrUtil.equalsAny(response.getSubCode(), "PAYMENT_INFO_INCONSISTENCY", "SYSTEM_ERROR", "ACQ.SYSTEM_ERROR")) {
            ...
            if (StrUtil.equalsAny(response.getStatus(), "REFUND", "FAIL")) { // 转账到银行卡会出现 "REFUND" "FAIL"
                return PayTransferRespDTO.closedOf(response.getSubCode(), response.getSubMsg(),
                    reqDTO.getOutTransferNo(), response);
            }
    ...
    @Override
    protected PayTransferRespDTO doGetTransfer(String outTradeNo, PayTransferType type) throws Throwable {
        // 1.1 构建 AlipayFundTransCommonQueryModel
        AlipayFundTransCommonQueryModel model = new AlipayFundTransCommonQueryModel();
        model.setProductCode(type == PayTransferType.BANK_CARD ? "TRANS_BANKCARD_NO_PWD" : "TRANS_ACCOUNT_NO_PWD");
        ...
        if (response.isSuccess()) {
            ...
            if (StrUtil.equalsAny(response.getSubCode(), "ORDER_NOT_EXIST", "SYSTEM_ERROR", "ACQ.SYSTEM_ERROR")) {
