 ...
 * @Description: 微信支付【付款码支付】的 PayClient 实现类
 * 档：<a href="https://pay.weixin.qq.com/wiki/doc/api/micropay.php?chapter=9_10&index=1">付款码支付</a>
...
public class WxBarPayClient extends AbstractWxPayClient {
    ...
    public WxBarPayClient(String channelId, WxPayClientConfig config) {
        super(channelId, PayType.WX_BAR.getKey(), config);
    }
        ...
        for (int i = 1; i < Byte.MAX_VALUE; i++) {
            try {
                ...
                return PayOrderRespDTO.successOf(response.getTransactionId(), response.getOpenid(), parseDateV2(response.getTimeEnd()),
                    ...
                    .setDisplayMode(PayOrderDisplayMode.BAR_CODE.getKey());
    ...
    static String getAuthCode(PayOrderUnifiedReqDTO reqDTO) {
        ...
        if (StrUtil.isEmpty(authCode)) {
            throw new CustomException("支付请求的 authCode 不能为空！");
        }
        return authCode;
    }
