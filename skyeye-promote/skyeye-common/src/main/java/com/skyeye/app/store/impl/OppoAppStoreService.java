...
import com.skyeye.app.store.config.OppoConfig;
...
/**
 * @ClassName: OppoAppStoreService
 * @Description: oppo应用市场服务实现类--https://open.oppomobile.com/
...
@Slf4j
@Service
public class OppoAppStoreService implements AppStoreService {

    private static final String STORE_KEY = "oppo";
    ...
    public AppStoreSubmitResult submitApp(AppVersion appVersion, AppRelease appRelease, AppStore appStore) {
        log.info("开始提交应用到oppo应用市场，版本ID：{}，发布记录ID：{}", appVersion.getId(), appRelease.getId());

        try {
            // 构建oppo配置
            OppoConfig config = buildOppoConfig(appVersion, appRelease, appStore);
    ...
    public AppStoreStatusResult queryAppStatus(AppRelease appRelease, AppStore appStore) {
        log.info("查询oppo应用市场发布状态，发布记录ID：{}", appRelease.getId());

        try {
            // 构建oppo配置
            OppoConfig config = buildOppoConfigFromRelease(appRelease, appStore);
    ...
    public AppStoreCancelResult cancelApp(AppRelease appRelease, AppStore appStore) {
        log.info("取消oppo应用市场发布，发布记录ID：{}", appRelease.getId());
    ...
    public AppStoreRemoveResult removeApp(AppRelease appRelease, AppStore appStore) {
        log.info("下架oppo应用市场应用，发布记录ID：{}", appRelease.getId());
     ...
     * 构建oppo配置
     */
    private OppoConfig buildOppoConfig(AppVersion appVersion, AppRelease appRelease, AppStore appStore) {
        OppoConfig config = new OppoConfig();
        ...
        if (appStore != null && StrUtil.isNotEmpty(appStore.getApiConfig())) {
            try {
                ...
                config.setApiUrl(configJson.getStr("apiUrl", "https://api.oppo.com"));
                config.setAuthUrl(configJson.getStr("authUrl", "https://api.oppo.com/oauth2/token"));
            } catch (Exception e) {
                log.warn("解析oppo应用商店配置失败，使用默认配置", e);
            }
        }
     ...
     * 从发布记录构建oppo配置
     */
    private OppoConfig buildOppoConfigFromRelease(AppRelease appRelease, AppStore appStore) {
        OppoConfig config = new OppoConfig();
     ...
     * 获取访问令牌
     */
    private String getAccessToken(OppoConfig config) {
        try {
            ...
            if (response.getStatus() == 200) {
                ...
                log.error("获取oppo访问令牌失败，状态码：{}，响应：{}", response.getStatus(), response.body());
                return null;
            }
        } catch (Exception e) {
            log.error("获取oppo访问令牌异常", e);
            return null;
        }
    }
     ...
     * 提交应用
     */
    private AppStoreSubmitResult doSubmitApp(OppoConfig config, String accessToken) {
        try {
            Map<String, Object> params = new HashMap<>();
            ...
            if (response.getStatus() == 200) {
                ...
                log.info("oppo应用提交成功，提交ID：{}，状态：{}", submissionId, status);
                return new AppStoreSubmitResult(true, "提交成功", submissionId, status);
            } else {
                log.error("oppo应用提交失败，状态码：{}，响应：{}", response.getStatus(), response.body());
                return new AppStoreSubmitResult(false, "提交失败：" + response.body());
            }
        } catch (Exception e) {
            log.error("oppo应用提交异常", e);
     ...
     * 查询应用状态
     */
    private AppStoreStatusResult doQueryAppStatus(OppoConfig config, String accessToken) {
        try {
            ...
            if (response.getStatus() == 200) {
                ...
                log.info("oppo应用状态查询成功，状态：{}，审核状态：{}", status, reviewStatus);
                return new AppStoreStatusResult(true, "查询成功", status, reviewStatus, reviewMessage);
            } else {
                log.error("oppo应用状态查询失败，状态码：{}，响应：{}", response.getStatus(), response.body());
                return new AppStoreStatusResult(false, "查询失败：" + response.body());
            }
        } catch (Exception e) {
            log.error("oppo应用状态查询异常", e);
     ...
     * 取消应用发布
     */
    private AppStoreCancelResult doCancelApp(OppoConfig config, String accessToken) {
        try {
            ...
            if (response.getStatus() == 200) {
                log.info("oppo应用取消发布成功");
                return new AppStoreCancelResult(true, "取消发布成功");
            } else {
                log.error("oppo应用取消发布失败，状态码：{}，响应：{}", response.getStatus(), response.body());
                return new AppStoreCancelResult(false, "取消发布失败：" + response.body());
            }
        } catch (Exception e) {
            log.error("oppo应用取消发布异常", e);
     ...
     * 下架应用
     */
    private AppStoreRemoveResult doRemoveApp(OppoConfig config, String accessToken) {
        try {
            ...
            if (response.getStatus() == 200) {
                ...
                log.error("oppo应用下架失败，状态码：{}，响应：{}", response.getStatus(), response.body());
                return new AppStoreRemoveResult(false, "下架失败：" + response.body());
            }
        } catch (Exception e) {
            log.error("oppo应用下架异常", e);
