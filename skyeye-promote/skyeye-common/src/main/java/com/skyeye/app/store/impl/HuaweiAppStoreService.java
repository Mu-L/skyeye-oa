...
import com.skyeye.app.store.config.HuaweiConfig;
...
/**
 * @ClassName: HuaweiAppStoreService
 * @Description: 华为应用市场服务实现类--https://developer.huawei.com/consumer/cn/
...
@Slf4j
@Service
public class HuaweiAppStoreService implements AppStoreService {

    private static final String STORE_KEY = "huawei";
    ...
    public AppStoreSubmitResult submitApp(AppVersion appVersion, AppRelease appRelease, AppStore appStore) {
        log.info("开始提交应用到华为应用市场，版本ID：{}，发布记录ID：{}", appVersion.getId(), appRelease.getId());

        try {
            // 构建华为配置
            HuaweiConfig config = buildHuaweiConfig(appVersion, appRelease, appStore);
    ...
    public AppStoreStatusResult queryAppStatus(AppRelease appRelease, AppStore appStore) {
        log.info("查询华为应用市场发布状态，发布记录ID：{}", appRelease.getId());

        try {
            // 构建华为配置
            HuaweiConfig config = buildHuaweiConfigFromRelease(appRelease, appStore);
    ...
    public AppStoreCancelResult cancelApp(AppRelease appRelease, AppStore appStore) {
        log.info("取消华为应用市场发布，发布记录ID：{}", appRelease.getId());
    ...
    public AppStoreRemoveResult removeApp(AppRelease appRelease, AppStore appStore) {
        log.info("下架华为应用市场应用，发布记录ID：{}", appRelease.getId());
     ...
     * 构建华为配置
     */
    private HuaweiConfig buildHuaweiConfig(AppVersion appVersion, AppRelease appRelease, AppStore appStore) {
        ...
        if (appStore != null && StrUtil.isNotEmpty(appStore.getApiConfig())) {
            try {
                ...
                config.setApiUrl(configJson.getStr("apiUrl", "https://connect-api.cloud.huawei.com/api"));
                config.setAuthUrl(configJson.getStr("authUrl", "https://oauth-login.cloud.huawei.com/oauth2/v3/token"));
            } catch (Exception e) {
                log.warn("解析华为应用商店配置失败，使用默认配置", e);
            }
        }
     ...
     * 从发布记录构建华为配置
     */
    private HuaweiConfig buildHuaweiConfigFromRelease(AppRelease appRelease, AppStore appStore) {
        HuaweiConfig config = new HuaweiConfig();
     ...
     * 获取访问令牌
     */
    private String getAccessToken(HuaweiConfig config) {
        try {
            ...
            if (response.getStatus() == 200) {
                ...
                log.error("获取华为访问令牌失败，状态码：{}，响应：{}", response.getStatus(), response.body());
                return null;
            }
        } catch (Exception e) {
            log.error("获取华为访问令牌异常", e);
            return null;
        }
    }
     ...
     * 提交应用
     */
    private AppStoreSubmitResult doSubmitApp(HuaweiConfig config, String accessToken) {
        try {
            Map<String, Object> params = new HashMap<>();
            ...
            if (response.getStatus() == 200) {
                ...
                log.info("华为应用提交成功，提交ID：{}，状态：{}", submissionId, status);
                return new AppStoreSubmitResult(true, "提交成功", submissionId, status);
            } else {
                log.error("华为应用提交失败，状态码：{}，响应：{}", response.getStatus(), response.body());
                return new AppStoreSubmitResult(false, "提交失败：" + response.body());
            }
        } catch (Exception e) {
            log.error("华为应用提交异常", e);
     ...
     * 查询应用状态
     */
    private AppStoreStatusResult doQueryAppStatus(HuaweiConfig config, String accessToken) {
        try {
     ...
     * 取消应用发布
     */
    private AppStoreCancelResult doCancelApp(HuaweiConfig config, String accessToken) {
        try {
            ...
            if (response.getStatus() == 200) {
                ...
                log.error("华为应用取消发布失败，状态码：{}，响应：{}", response.getStatus(), response.body());
                return new AppStoreCancelResult(false, "取消发布失败：" + response.body());
            }
        } catch (Exception e) {
            log.error("华为应用取消发布异常", e);
     ...
     * 下架应用
     */
    private AppStoreRemoveResult doRemoveApp(HuaweiConfig config, String accessToken) {
        try {
            ...
            if (response.getStatus() == 200) {
                ...
                log.error("华为应用下架失败，状态码：{}，响应：{}", response.getStatus(), response.body());
                return new AppStoreRemoveResult(false, "下架失败：" + response.body());
            }
        } catch (Exception e) {
            log.error("华为应用下架异常", e);
