...
import com.skyeye.app.store.config.XiaomiConfig;
...
/**
 * @ClassName: XiaomiAppStoreService
 * @Description: 小米应用市场服务实现类-https://dev.mi.com/
...
@Slf4j
@Service
public class XiaomiAppStoreService implements AppStoreService {

    private static final String STORE_KEY = "xiaomi";
    ...
    public AppStoreSubmitResult submitApp(AppVersion appVersion, AppRelease appRelease, AppStore appStore) {
        log.info("开始提交应用到小米应用市场，版本ID：{}，发布记录ID：{}", appVersion.getId(), appRelease.getId());

        try {
            // 构建小米配置
            XiaomiConfig config = buildXiaomiConfig(appVersion, appRelease, appStore);
    ...
    public AppStoreStatusResult queryAppStatus(AppRelease appRelease, AppStore appStore) {
        log.info("查询小米应用市场发布状态，发布记录ID：{}", appRelease.getId());

        try {
            // 构建小米配置
            XiaomiConfig config = buildXiaomiConfigFromRelease(appRelease, appStore);
    ...
    public AppStoreCancelResult cancelApp(AppRelease appRelease, AppStore appStore) {
        log.info("取消小米应用市场发布，发布记录ID：{}", appRelease.getId());
    ...
    public AppStoreRemoveResult removeApp(AppRelease appRelease, AppStore appStore) {
        log.info("下架小米应用市场应用，发布记录ID：{}", appRelease.getId());
     ...
     * 构建小米配置
     */
    private XiaomiConfig buildXiaomiConfig(AppVersion appVersion, AppRelease appRelease, AppStore appStore) {
        XiaomiConfig config = new XiaomiConfig();
        ...
        if (appStore != null && StrUtil.isNotEmpty(appStore.getApiConfig())) {
            try {
                ...
                config.setApiUrl(configJson.getStr("apiUrl", "https://api.developer.xiaomi.com"));
                config.setAuthUrl(configJson.getStr("authUrl", "https://account.xiaomi.com/oauth2/authorize"));
            } catch (Exception e) {
                log.warn("解析小米应用商店配置失败，使用默认配置", e);
            }
        }
     ...
     * 从发布记录构建小米配置
     */
    private XiaomiConfig buildXiaomiConfigFromRelease(AppRelease appRelease, AppStore appStore) {
        XiaomiConfig config = new XiaomiConfig();
     ...
     * 获取访问令牌
     */
    private String getAccessToken(XiaomiConfig config) {
        try {
            ...
            HttpResponse response = HttpRequest.post(config.getAuthUrl() + "/token")
                .form(params)
                .execute();
            ...
            if (response.getStatus() == 200) {
                ...
                log.error("获取小米访问令牌失败，状态码：{}，响应：{}", response.getStatus(), response.body());
                return null;
            }
        } catch (Exception e) {
            log.error("获取小米访问令牌异常", e);
            return null;
        }
    }
     ...
     * 提交应用
     */
    private AppStoreSubmitResult doSubmitApp(XiaomiConfig config, String accessToken) {
        try {
            Map<String, Object> params = new HashMap<>();
            ...
            if (response.getStatus() == 200) {
                ...
                log.info("小米应用提交成功，提交ID：{}，状态：{}", submissionId, status);
                return new AppStoreSubmitResult(true, "提交成功", submissionId, status);
            } else {
                log.error("小米应用提交失败，状态码：{}，响应：{}", response.getStatus(), response.body());
                return new AppStoreSubmitResult(false, "提交失败：" + response.body());
            }
        } catch (Exception e) {
            log.error("小米应用提交异常", e);
     ...
     * 查询应用状态
     */
    private AppStoreStatusResult doQueryAppStatus(XiaomiConfig config, String accessToken) {
        try {
     ...
     * 取消应用发布
     */
    private AppStoreCancelResult doCancelApp(XiaomiConfig config, String accessToken) {
        try {
            ...
            if (response.getStatus() == 200) {
                ...
                log.error("小米应用取消发布失败，状态码：{}，响应：{}", response.getStatus(), response.body());
                return new AppStoreCancelResult(false, "取消发布失败：" + response.body());
            }
        } catch (Exception e) {
            log.error("小米应用取消发布异常", e);
     ...
     * 下架应用
     */
    private AppStoreRemoveResult doRemoveApp(XiaomiConfig config, String accessToken) {
        try {
            ...
            if (response.getStatus() == 200) {
                ...
                log.error("小米应用下架失败，状态码：{}，响应：{}", response.getStatus(), response.body());
                return new AppStoreRemoveResult(false, "下架失败：" + response.body());
            }
        } catch (Exception e) {
            log.error("小米应用下架异常", e);
