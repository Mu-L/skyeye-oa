...
@Service
@SkyeyeService(name = "借出申请", groupName = "借出申请")
public class ProductLeadServiceImpl extends SkyeyeErpOrderServiceImpl<ProductLeadDao, ProductLead> implements ProductLeadService {

    @Autowired
    private ProductLeadPutService productLeadPutService;
    ...
    public QueryWrapper<ProductLead> getQueryWrapper(CommonPageInfo commonPageInfo) {
        ...
        if (StrUtil.isNotEmpty(commonPageInfo.getHolderId())) {
            queryWrapper.eq(MybatisPlusUtil.toColumns(ProductLead::getHolderId), commonPageInfo.getHolderId());
        }
        return queryWrapper;
    }
    ...
    public void createPrepose(ProductLead entity) {
        ...
        QueryWrapper<ProductLead> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq(MybatisPlusUtil.toColumns(ProductLead::getHolderId), entity.getHolderId());
        queryWrapper.ne(MybatisPlusUtil.toColumns(ProductLead::getState), FlowableStateEnum.PASS.getKey());
        ProductLead productLead = getOne(queryWrapper);
        if (ObjectUtil.isNotEmpty(productLead)) {
            throw new CustomException("该用户已存在借出申请单，不能重复提交");
        }
    }
