...
*/
     ...
     * @param question
     */
    public void getQuestionOptionListMation(Map<String, Object> question) {
        //获取题目类型
    ...
    public List<Map<String, Object>> setLogics(String quId, String logicStr, String userId) {
        ...
        if (!insertList.isEmpty()) {
            examDao.addQuestionLogicsMationList(insertList);
        }
        if (!editList.isEmpty()) {
            examDao.editQuestionLogicsMationList(editList);
        }
    ...
    public void deleteQuestionMationById(InputObject inputObject, OutputObject outputObject) {
        ...
        if (question != null) {
            if (question.get("surveyState").toString().equals("0")) {
                // 设计状态，执行物理删除
                ...
                examDao.deleteQuestionMationById(map);
            } else {
                // 执行中或者结束，执行逻辑删除问题
                examDao.deleteLogicQuestionMationById(map);
            }
            // 修改该问题排序后面的序号减1
            examDao.updateQuestionOrderByIdByQuId(question);
        }
    }
    ...
    public void deleteQuestionChenColumnMationById(InputObject inputObject, OutputObject outputObject) {
        ...
        if (option != null) {
            if (option.get("surveyState").toString().equals("0")) {
                // 设计状态，执行物理删除
                examDao.deleteQuestionChenColumnMationById(map);
            } else {
                // 执行中或者结束，执行逻辑删除问题
                examDao.deleteLogicQuestionChenColumnMationById(map);
            }
        }
    }
    ...
    public void deleteQuestionChenRowMationById(InputObject inputObject, OutputObject outputObject) {
        ...
        if (option != null) {
            if (option.get("surveyState").toString().equals("0")) {
                // 设计状态，执行物理删除
                examDao.deleteQuestionChenRowMationById(map);
            } else {
                // 执行中或者结束，执行逻辑删除问题
                examDao.deleteLogicQuestionChenRowMationById(map);
            }
        }
    }
    ...
    public void deleteQuestionRadioOptionMationById(InputObject inputObject, OutputObject outputObject) {
        ...
        if (option != null) {
            if (option.get("surveyState").toString().equals("0")) {
                // 设计状态，执行物理删除
                examDao.deleteQuestionRadioOptionMationById(map);
            } else {
                // 执行中或者结束，执行逻辑删除问题
                examDao.deleteLogicQuestionRadioOptionMationById(map);
            }
        }
    }
    ...
    public void deleteQuestionChedkBoxOptionMationById(InputObject inputObject, OutputObject outputObject) {
        ...
        if (option != null) {
            if (option.get("surveyState").toString().equals("0")) {
                // 设计状态，执行物理删除
                examDao.deleteQuestionChedkBoxOptionMationById(map);
            } else {
                // 执行中或者结束，执行逻辑删除问题
                examDao.deleteLogicQuestionChedkBoxOptionMationById(map);
            }
        }
    }
    ...
    public void deleteQuestionScoreOptionMationById(InputObject inputObject, OutputObject outputObject) {
        ...
        if (option != null) {
            if (option.get("surveyState").toString().equals("0")) {
                // 设计状态，执行物理删除
                examDao.deleteQuestionScoreOptionMationById(map);
            } else {
                // 执行中或者结束，执行逻辑删除问题
                examDao.deleteLogicQuestionScoreOptionMationById(map);
            }
        }
    }
    ...
    public void deleteQuestionOrderOptionMationById(InputObject inputObject, OutputObject outputObject) {
        ...
        if (option != null) {
            if (option.get("surveyState").toString().equals("0")) {
                // 设计状态，执行物理删除
                examDao.deleteQuestionOrderOptionMationById(map);
            } else {
                // 执行中或者结束，执行逻辑删除问题
                examDao.deleteLogicQuestionOrderOptionMationById(map);
            }
        }
    }
    ...
    public void editExamStateToReleaseById(InputObject inputObject, OutputObject outputObject) {
        ...
        if (surveyMation != null && !surveyMation.isEmpty()) {
            ...
            if ("0".equals(surveyMation.get("surveyState").toString())) {
                // 设计状态可以发布试卷
                // 获取试卷中的题
                List<Map<String, Object>> questions = examDao.queryQuestionListByBelongId(map);
    ...
    public void deleteExamMationById(InputObject inputObject, OutputObject outputObject) {
        Map<String, Object> map = inputObject.getParams();
        // 删除试卷
        examDao.deleteExamMationById(map);
    }
    ...
    public Map<String, Object> getQuestionOptionReportListMation(Map<String, Object> question) {
        ...
        } else if (quType.equals(QuType.ANSWER.getActionName())) {
            // 多行填空题
        ...
        } else if (quType.equals(QuType.ENUMQU.getActionName())) {
            // 枚举题
    ...
    public void insertAnswerExamMationByIp(InputObject inputObject, OutputObject outputObject) {
        ...
        if (judgeWhetherExaming(map, inputObject.getLogParams(), outputObject)) {
            map.put("ip", ToolUtil.getIpByRequest(PutObject.getRequest()));//获取答卷ip
     ...
     * @return
     */
    public boolean judgeWhetherExaming(Map<String, Object> map, Map<String, Object> user, OutputObject outputObject) {
        Map<String, Object> surveyMation = examDao.queryExamMationById(map);//获取试卷信息
     ...
     * @return
     */
    public Map<String, Map<String, Object>> buildSaveExamMap(HttpServletRequest request, Map<String, Object> params) {
        // 是非
     ...
     * @param quAnswerFile
     */
    private void saveAnswer(Map<String, Object> surveyAnswer, Map<String, Map<String, Object>> quMaps, String stuId, String quAnswerFile) {
        // 获取试卷信息
        ...
        // 分钟
        surveyAnswer.put("totalTime", DateUtil.getDistanceMinute(surveyAnswer.get("bgAnDate").toString(), surveyAnswer.get("endAnDate").toString()));
     ...
     * @param stuId    学生id
     */
    private void saveAnswerFileUrl(String answerId, String jsonStr, String stuId) {
     ...
     * @return
     */
    public void getQuestionAnswerListMation(Map<String, Object> question, String answerId) {
        // 获取题目类型
        String quType = QuType.getActionName(Integer.parseInt(question.get("quType").toString()));
