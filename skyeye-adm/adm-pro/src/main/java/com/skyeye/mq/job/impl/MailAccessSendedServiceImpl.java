...
public class MailAccessSendedServiceImpl implements RocketMQListener<String> {
    ...
    public void onMessage(String data) {
        ...
        try {
            ...
            Folder folder = ToolUtil.getFolderByServer(host, emailReceiptServerPort, username, password, storeType, "Sent Messages");
            ...
            //获取当前邮箱已有的邮件
            List<Map<String, Object>> emailHasMail = emailDao.queryEmailListByEmailFromAddress(username, EmailState.NORMAL.getKey(), tenantId);
            ...
            for (int i = 0; i < message.length; i++) {
                ...
                if (beans.size() >= 20) {//每20条数据保存一次
                    ...
                    emailHasMail = emailDao.queryEmailListByEmailFromAddress(username, EmailState.NORMAL.getKey(), tenantId);
                }
            }
            if (!beans.isEmpty()) {
                emailDao.insertEmailListToServer(beans, tenantId);
